...
export function PriceChart({ safeS, activeTF, ew, vsa, smcData,
  ...
  // FIX: guard ctrlLayers undefined
  ...
  return (
    <div style={{ background:'#060d18', border:'1px solid #1e3050',
      ...
      <div style={{ display:'flex', justifyContent:'space-between',
        ...
        <div style={{ display:'flex', gap:7, alignItems:'center' }}>
          ...
          <span style={{ fontFamily:'monospace', fontSize:13,
            fontWeight:700, color:comexUp?C.green:C.red }}>
            ${(safeS.comex||0).toFixed(3)}
          </span>
          <span style={{ fontSize:9, color:comexUp?C.green:C.red }}>
            {comexUp?'▲':'▼'}
            {Math.abs(safeS.comex_chg_pct||0).toFixed(2)}%
          </span>
      ...
      {/* TVChart — single ref, guard layers */}
      <TVChart
        bars={bars}
        activeTF={activeTF}
        ew={ew}
      ...
      <div style={{ textAlign:'center', fontSize:8,
        ...
        TradingView Lightweight Charts · {activeTF} ·{' '}
        {barCount} candles ·{' '}
...
export function ControlPanel({ ctrlLayers, onToggleLayer,
  ...
  return (
    <div style={{ background:'#060d18',
      border:`1px solid ${layers.ai_detection
        ?C.purple+'66':'#1e3050'}`,
      borderRadius:11, padding:'11px 13px' }}>
      ...
      {/* AI Detection master */}
      <div style={{
        background:layers.ai_detection?`${C.purple}12`:'#0a1520',
        border:`0.5px solid ${layers.ai_detection
          ?C.purple:'#1e3050'}`,
        ...
        <div style={{ display:'flex', justifyContent:'space-between',
          ...
          <div>
            <div style={{ fontSize:10, fontWeight:600,
              color:layers.ai_detection?C.purple:'#e2e8f0' }}>
              🤖 AI Detection
            </div>
          ...
          <ToggleSwitch label="" small
            active={layers.ai_detection!==false}
        ...
        {layers.ai_detection&&(
          <div style={{ display:'flex', gap:4,
            ...
            <Bdg
              label={wyckoff?.label?.replace('Phase ','')||'?'}
              col={C.amber}/>
          </div>
        )}
      </div>
        ...
        {activeTool&&(
          <div style={{ marginTop:6, fontSize:8, color:C.blue,
            ...
            <span style={{
              ...
              fontSize:10 }}>✏️</span>
            Đang vẽ{' '}
            <b style={{ color:'#e2e8f0' }}>{activeTool}</b>
