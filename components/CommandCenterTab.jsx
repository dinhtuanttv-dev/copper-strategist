    ...
    <Card>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 320, overflowY: 'auto' }}>
        ...
        {filtered.map((n, i) => {
          ...
          return (
            <div key={i} style={{ background: 'var(--card2)', borderRadius: 8, padding: '7px 10px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'flex-start', cursor: 'pointer' }} onClick={() => handleExpand(n.title)}>
                <span style={{ fontSize: 11, flex: 1 }}>{n.title}</span>
                <span style={{ color: dirColor(n.direction), flexShrink: 0, fontSize: 11 }}>{n.direction === 'bull' ? '🟢' : n.direction === 'bear' ? '🔴' : '🟡'}</span>
              </div>
              <div style={{ fontSize: 9, color: 'var(--muted)', marginTop: 2 }}>{n.source} · {n.age}</div>

              {isOpen && (
                <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                    <span style={{ fontSize: 9, color: 'var(--muted)' }}>Độ liên quan</span>
                    <div style={{ flex: 1, height: 4, background: 'var(--border)', borderRadius: 2 }}>
                      <div style={{ width: `${confidencePct}%`, height: '100%', background: '#378ADD', borderRadius: 2 }} />
                    </div>
                    <span style={{ fontSize: 9, color: '#378ADD' }}>{n.score?.toFixed(1)}/10</span>
                  </div>

                  {detail?.loading && <div style={{ fontSize: 10, color: 'var(--muted)' }}>⟳ Đang phân tích...</div>}
                  {detail?.analysis && (
                    <>
                      <div style={{ fontSize: 9, color: 'var(--muted)', marginBottom: 3 }}>
                        Phân tích AI (1 nguồn — Claude){detail.fromCache ? ' · từ cache' : ''}
                      </div>
                      <div style={{ fontSize: 11, lineHeight: 1.6 }}>{detail.analysis}</div>
                    </>
                  )}
                  {detail?.error && <div style={{ fontSize: 10, color: '#E5484D' }}>Không phân tích được: {detail.error}</div>}

                  {n.link && (
                    <a href={n.link} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 6, fontSize: 10, color: '#378ADD' }}>
                      Đọc bài gốc ↗
                    </a>
                  )}
                </div>
