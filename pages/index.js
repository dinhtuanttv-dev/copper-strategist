...
function NewsPanel({ news, loading, onRefresh }) {
  if (loading) return (
    <div style={{ padding:'10px 0', textAlign:'center',
      fontSize:10, color:'var(--muted)' }}>⟳ Đang tải tin tức...</div>
  );
            ...
            <div style={{ display:'flex', justifyContent:'space-between',
              ...
              <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                <span style={{ fontSize:7, padding:'1px 4px', borderRadius:3,
                  background:imp+'22', color:imp, fontWeight:700 }}>
  ...
  return (
    <svg width="180" height="105" viewBox="0 0 180 105">
      {segs.map(({s,e,c},i)=>{
        ...
        return(<path key={i} d={`M${cx} ${cy}L${x1} ${y1}A${R} ${R} 0 ${e-s>50?1:0} 1 ${x2} ${y2}Z`} fill={c} opacity={0.75}/>);
      })}
  ...
  const fetchPrice = useCallback(async () => {
    ...
    try {
      ...
      if (d.comex) {
        patchPrice(d);
        setTsPrice(Date.now());
        aLog(`✅ COMEX $${d.comex.toFixed(3)}/lb`);
      } else {
        aLog('⚠️ Không lấy được giá');
      }
    ...
    try {
      const r = await fetch('/api/claude', {
        ...
        body: JSON.stringify({
          model:'claude-sonnet-4-5', max_tokens:500,
          messages:[{ role:'user', content:
            `Search latest CFTC COT copper managed money long contracts, fear greed index, copper gold ratio, DXY index value.
...
Đoạn 1: Tổng quan thị trường. Đoạn 2: Điểm mạnh/yếu. Đoạn 3: Hành động cụ thể + giá mục tiêu.`
          }],
        }),
      });
      const d    = await r.json();
  ...
  return (
    <Layout tab={tab} onTabChange={setTab} priceData={s} verdict={verdict}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',
        ...
        {[
          {lbl:'COMEX',   val:`$${s.comex?.toFixed(3)||'6.070'}`, chg:s.comex_chg_pct, col:comexUp?A.green:A.red},
        ...
        <div style={{ display:'grid', gap:10 }}>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:10 }}>
            ...
            <Card glow={verdict.verdictCol}>
              ...
              <div style={{ background:verdict.verdictCol+'18',
                ...
                <div style={{ fontSize:11, fontWeight:800, color:verdict.verdictCol }}>
                  {verdict.verdictLabel}
                </div>
                <div style={{ fontSize:36, fontWeight:800, color:verdict.verdictCol,
              ...
              {verdictText && (
                <div style={{ marginTop:8, background:A.purple+'08',
                  ...
                  color:'var(--text)', lineHeight:1.8, whiteSpace:'pre-line' }}>
                  {verdictText}
                </div>
              )}
            </Card>
          ...
          <Card glow={comexUp?A.green:A.red}>
            <div style={{ display:'flex', justifyContent:'space-between',
              ...
              <div style={{ fontSize:11, fontWeight:700 }}>📈 COMEX PRICE CHART</div>
        ...
        <div style={{ display:'grid', gap:10 }}>
          <div style={{ display:'grid',
            ...
            <Card glow={ew.failure?A.red:A.blue}>
              ...
              <div style={{ background:(ew.failure?A.red:A.blue)+'14',
                borderRadius:8, padding:'8px', marginBottom:8 }}>
                <div style={{ fontSize:12, fontWeight:800,
                  color:ew.failure?A.red:A.cyan }}>{ew.label}</div>
                <div style={{ fontSize:10, color:'var(--muted)',
                  marginTop:3 }}>{ew.scenario}</div>
              </div>
              {[
                ...
                <div key={i} style={{ display:'flex', alignItems:'center', gap:7,
                  ...
                  <span style={{ fontSize:13, fontWeight:700, color:f.col,
                    fontFamily:'var(--font-mono)' }}>${f.val?.toFixed(3)||'–'}</span>
                  {Math.abs((s.comex||6.07)-f.val)<0.05 && (
                    <Chip label="← NOW" color={f.col} size={8} />
                  )}
                </div>
              ))}
            </Card>
            ...
            <Card glow={vsa.bullish?A.green:vsa.bearish?A.red:A.amber}>
              ...
              <div style={{ background:(vsa.bullish?A.green:vsa.bearish?A.red:A.amber)+'14',
                ...
                <div style={{ fontSize:11, fontWeight:800,
                  color:vsa.bullish?A.green:vsa.bearish?A.red:A.amber }}>
                  {vsa.meta.label}
                </div>
                <div style={{ fontSize:9, color:'var(--muted)',
                  marginTop:2 }}>{vsa.meta.desc}</div>
              </div>
              {vsa.bars.slice(-5).reverse().map((b,i) => {
                const VC = {
                  ABSORPTION_BULL:A.green, ABSORPTION_BEAR:A.red,
                  STOPPING_VOLUME:A.amber, UPTHRUST:A.red, SPRING:A.green,
                  NO_DEMAND:A.orange, NO_SUPPLY:A.teal, NEUTRAL:'var(--muted)',
                };
                ...
                return (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:6,
                    ...
                    <span style={{ fontSize:11, fontWeight:700,
                      color:b.up?A.green:A.red }}>${b.comex?.toFixed(3)}</span>
                    <span style={{ fontSize:9, color:'var(--muted)',
                      flex:1 }}>{b.volRatio}×</span>
          ...
          <Card glow={pricePattern.meta.col}>
            <div style={{ fontSize:11, fontWeight:700, marginBottom:8 }}>
              🔍 PRICE PATTERN · <span style={{ color:pricePattern.meta.col }}>
            ...
            <div style={{ display:'grid',
              ...
              <div style={{ background:pricePattern.meta.col+'12',
                ...
                <div style={{ fontSize:12, fontWeight:800,
                  color:pricePattern.meta.col, marginBottom:5 }}>
                  {pricePattern.meta.name}
                </div>
                <div style={{ fontSize:10, color:'var(--text)',
                  lineHeight:1.7, marginBottom:7 }}>{pricePattern.desc}</div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:7 }}>
                  <div style={{ background:'var(--card2)', borderRadius:6, padding:'6px 8px' }}>
                    ...
                    <div style={{ fontSize:16, fontWeight:800,
                      color:pricePattern.meta.col }}>{pricePattern.reliability}%</div>
                  </div>
                  <div style={{ background:'var(--card2)', borderRadius:6, padding:'6px 8px' }}>
                    <div style={{ fontSize:8, color:'var(--muted)' }}>Target</div>
                    <div style={{ fontSize:16, fontWeight:800,
                      color:pricePattern.meta.col }}>${pricePattern.target}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <Card glow={stress.sevCol}>
            ...
            <div style={{ display:'grid',
              ...
              {bsEvents.slice(0,4).map((e,i) => (
                <div key={i} style={{ background:'var(--card2)',
                  border:`1px solid ${e.col}35`, borderRadius:8,
                  padding:'8px 10px' }}>
                  <div style={{ display:'flex', justifyContent:'space-between',
                    marginBottom:4 }}>
                    <span style={{ fontSize:10, fontWeight:700,
                      color:e.col }}>📍 {e.region}</span>
                    <Chip label={`${e.impact}/100`} color={e.col} size={8} />
                  </div>
                  <div style={{ fontSize:9, color:'var(--text)',
                    marginBottom:5 }}>{e.event}</div>
                  <div style={{ background:'var(--border)', borderRadius:3,
                    height:3, overflow:'hidden' }}>
                    <div style={{ width:`${e.impact}%`, height:'100%',
                      background:e.col }} />
                  </div>
                </div>
              ))}
        ...
        <div style={{ display:'grid', gap:10 }}>
          <Card glow={verdict.verdictCol}>
            ...
            <div style={{ background:verdict.verdictCol+'18',
              ...
              <div style={{ fontSize:12, fontWeight:800,
                color:verdict.verdictCol, marginBottom:4 }}>{verdict.verdictLabel}</div>
              <div style={{ fontSize:40, fontWeight:800,
                color:verdict.verdictCol, lineHeight:1 }}>{verdict.final}</div>
            ...
            <FetchBtn onClick={() => doVerdict(verdict,ti,mh,bias)}
              loading={loadVerdict} label="🧠 AI Phân tích chuyên sâu"
              icon="🧠" col={A.purple} />
            {verdictText && (
              <div style={{ marginTop:10, background:A.purple+'08',
                borderRadius:8, padding:'12px', fontSize:11,
                color:'var(--text)', lineHeight:1.9, whiteSpace:'pre-line' }}>
                {verdictText}
              </div>
            )}
          </Card>
        ...
        <div style={{ display:'grid', gap:10 }}>
          <Card>
            <div style={{ fontSize:12, fontWeight:800, marginBottom:10 }}>
              🌏 KẾ HOẠCH PHIÊN GIAO DỊCH
            </div>
            <div style={{ display:'grid',
              gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:8 }}>
              {[
                {lbl:'Entry A',  val:`$${(s.prev_low||5.89+0.05).toFixed(3)}–$${(s.prev_low||5.89+0.12).toFixed(3)}`, col:A.green, icon:'✅'},
                {lbl:'TP1',      val:`$${s.tp1||6.32}`,  col:A.teal,  icon:'🎯'},
                {lbl:'TP2',      val:`$${s.tp2||6.58}`,  col:A.cyan,  icon:'🎯'},
                {lbl:'Stop Loss',val:`$${s.sl||5.72}`,   col:A.red,   icon:'🛑'},
              ].map((m,i) => (
                <div key={i} style={{ background:m.col+'12',
                  border:`1px solid ${m.col}44`,
                  borderRadius:8, padding:'10px 12px' }}>
                  <div style={{ fontSize:9, color:'var(--muted)',
                    marginBottom:3 }}>{m.icon} {m.lbl}</div>
                  <div style={{ fontSize:16, fontWeight:800, color:m.col,
                    fontFamily:'var(--font-mono)' }}>{m.val}</div>
                </div>
              ))}
            </div>
            <div style={{ marginTop:12, background:'var(--card2)',
              borderRadius:8, padding:'10px 12px' }}>
              <div style={{ fontSize:10, fontWeight:700, marginBottom:6 }}>
                📐 KEY LEVELS
              </div>
              {[
                {lbl:'Đỉnh cũ',   price:s.prev_high||6.12, col:A.orange, note:'Resistance'},
                {lbl:'COMEX now', price:s.comex||6.07,     col:A.cyan,   note:'Hiện tại'},
                {lbl:'Fib 0.618', price:ew.fib618,          col:A.amber,  note:'Hỗ trợ'},
                {lbl:'Đáy cũ',   price:s.prev_low||5.89,  col:A.green,  note:'Support'},
                {lbl:'Stop Loss', price:s.sl||5.72,         col:A.red,    note:'Exit'},
              ].map((lv,i) => (
                <div key={i} style={{ display:'flex', alignItems:'center',
                  gap:10, padding:'5px 0',
                  borderBottom:'1px solid var(--border)' }}>
                  <span style={{ fontSize:9, color:'var(--muted)', width:70 }}>{lv.lbl}</span>
                  <span style={{ fontSize:12, fontWeight:800, color:lv.col,
                    fontFamily:'var(--font-mono)', flex:1 }}>
                    ${lv.price?.toFixed(3)||'–'}
                  </span>
                  <span style={{ fontSize:8, color:'var(--muted)' }}>{lv.note}</span>
                </div>
              ))}
            </div>
          </Card>
