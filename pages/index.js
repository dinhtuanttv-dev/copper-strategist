    ...
    <div style={{ marginBottom:6 }}>
      ...
      <div style={{ background:'var(--border)', borderRadius:5, height:8, overflow:'hidden' }}>
        <div style={{ width:`${Math.min(100,Math.round(score/maxScore*100))}%`,
          height:'100%', background:`linear-gradient(90deg,${col}99,${col})`,
          borderRadius:5 }} />
      </div>
    </div>
  );
...
function Chip({ label, color, size=10, pulse }) {
  return (
    <span style={{ background:color+'22', color, border:`1px solid ${color}55`,
      borderRadius:6, padding:'2px 9px', fontSize:size, fontWeight:700,
      whiteSpace:'nowrap', animation:pulse?'glow 1.4s ease-in-out infinite':'' }}>
      {label}
    </span>
  );
}
...
function AutoRefreshBar({ nextPrice, nextFund, nextBS,
  loadPrice, loadInv, loadCOT, loadCal, loadBS }) {
  ...
  const items = [
    { icon:'⚡', lbl:'Giá',    next:nextPrice, loading:loadPrice,
      interval:RF.price, col:A.cyan },
    { icon:'📦', lbl:'Cơ bản', next:nextFund,
      loading:loadInv||loadCOT||loadCal, interval:RF.fund, col:A.teal },
    { icon:'🦢', lbl:'BS',     next:nextBS, loading:loadBS,
      interval:RF.bs, col:A.red },
  ];
  ...
  return (
    <div style={{ background:'var(--card2)', border:'1px solid var(--border)',
      ...
      <span style={{ fontSize:9, color:'var(--muted)', fontWeight:700, flexShrink:0 }}>
        ⏰ AUTO-REFRESH
      </span>
      {items.map((m,i) => {
        ...
        const label     = m.loading      ?'⟳ updating...'
                        : remaining<=0   ?'⟳ soon'
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:3 }}>
      ...
      <span style={{ fontSize:8, color:c, fontWeight:600 }}>
        {fmtTime(ts)} · {age}
      </span>
    </div>
  );
...
function NewsPanel({ news, loading, onRefresh }) {
  ...
  if (!news?.length) return (
    <div style={{ padding:'8px 0' }}>
      <FetchBtn onClick={onRefresh} loading={loading}
        label="Tải tin tức" icon="📰" col={A.blue} small />
    </div>
  );
  return (
    <div>
      {news.map((n,i) => {
        const col = n.direction==='bullish'?A.green
                  : n.direction==='bearish'?A.red:A.amber;
        const imp = n.impact==='high'?A.red
                  : n.impact==='medium'?A.amber:A.teal;
        return (
          <div key={i} style={{ background:'var(--card2)',
            ...
            <div style={{ display:'flex', justifyContent:'space-between',
              ...
              <div style={{ fontSize:10, fontWeight:700,
                color:'var(--text)', flex:1 }}>{n.title}</div>
              <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                <span style={{ fontSize:7, padding:'1px 4px', borderRadius:3,
                  background:imp+'22', color:imp,
                  fontWeight:700 }}>{n.impact?.toUpperCase()}</span>
                <span style={{ fontSize:7, padding:'1px 4px', borderRadius:3,
  ...
  return (
    <div>
      {sorted.map((ev,i) => {
        ...
        const col  = ev.impact==='high'?A.red
                   : ev.impact==='medium'?A.amber:A.teal;
        ...
        return (
          <div key={i} style={{ background:'var(--card2)',
            ...
            marginBottom:4, opacity:past?0.5:1 }}>
            <div style={{ display:'flex', justifyContent:'space-between',
              ...
              <div style={{ display:'flex', gap:5, alignItems:'center' }}>
                <span style={{ background:col+'22', color:col, borderRadius:4,
                  ...
                  fontWeight:800 }}>{ev.impact?.toUpperCase()}</span>
                <span style={{ fontSize:10, fontWeight:700,
                  color:'var(--text)' }}>{ev.name}</span>
              </div>
              <div style={{ fontSize:12, fontWeight:800,
                color:past?'var(--muted)':col,
            ...
            {!past&&ev.note&&(
              <div style={{ background:col+'10', borderRadius:4,
                padding:'2px 6px', fontSize:9, color:col }}>
                💡 {ev.note}
              </div>
            )}
          </div>
...
function Gauge({ value }) {
  ...
  const col = value<30?A.red:value<50?A.orange
            : value<70?A.amber:value<85?A.green:A.teal;
  return (
    <svg width="180" height="105" viewBox="0 0 180 105">
      ...
      <text x={cx} y={cy-16} textAnchor="middle"
        fill={col} fontSize={20} fontWeight={700}>{value}</text>
      <text x={cx} y={cy-2} textAnchor="middle"
        fill="var(--muted)" fontSize={8}>
        {value<30?'SỢ HÃI':value<50?'LO NGẠI'
         :value<70?'TRUNG LẬP':value<85?'THAM LAM':'EXTREME'}
      </text>
...
export default function Home() {
  ...
  const [futures]             = useState(DEF_FUTURES);
  ...
  const [nextFund,  setNextFund]  = useState(null);
  const [nextBS,    setNextBS]    = useState(null);
  ...
  const patchPrice = useCallback(f => {
    setS(prev => {
      const next = {...prev,...f};
      const lbl  = new Date().toLocaleDateString('vi-VN',
        {day:'2-digit',month:'2-digit'});
      const pc   = [...prev.priceChart];
      const last = pc[pc.length-1];
      if (last&&last.d!==lbl)
        pc.push({d:lbl,comex:next.comex,vol:next.session_vol||91000});
  ...
  const patchInv = useCallback(f => {
    setS(prev => {
      ...
      const w    = new Date().toLocaleDateString('vi-VN',
        {day:'2-digit',month:'2-digit'});
  ...
  const patchCOT = useCallback(f => {
    setS(prev => {
      ...
      if (f.mm_long!==undefined) {
        const w = new Date().toLocaleDateString('vi-VN',
          {day:'2-digit',month:'2-digit'});
        cot.push({w,mm:f.mm_long,
          comm:f.comm_short??prev.comm_short??-73000});
...
Đoạn 1: Tổng quan. Đoạn 2: Điểm mạnh/yếu. Đoạn 3: Hành động cụ thể + giá mục tiêu.`
  ...
  return (
    <Layout tab={tab} onTabChange={setTab} priceData={s} verdict={verdict}>
      ...
      {/* ── Auto-refresh bar ── */}
      <AutoRefreshBar
        nextPrice={nextPrice} nextFund={nextFund} nextBS={nextBS}
        loadPrice={loadPrice} loadInv={loadInv} loadCOT={loadCOT}
        loadCal={loadCal} loadBS={loadBS}
      />
      ...
      <div style={{ display:'grid',
        ...
        <div style={{ background:'var(--card2)',
          ...
          <div style={{ fontSize:15, fontWeight:800,
            color:comexUp?A.green:A.red,
            fontFamily:'var(--font-mono)' }}>
            ${s.comex?.toFixed(3)||'6.070'}
          </div>
        ...
        {[
          {lbl:'DXY',     val:`${s.dxy||99.8}`,     chg:s.dxy_chg,
           col:(s.dxy_chg||0)<0?A.green:A.red},
      ...
      {tab===4&&(
        <div style={{ display:'grid', gap:10 }}>
          ...
          <div style={{ background:liq.col+'18',
            border:`1.5px solid ${liq.col}`, borderRadius:10,
            padding:'10px 13px', display:'flex',
            alignItems:'center', gap:10 }}>
            <div style={{ fontSize:14 }}>💧</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:11, fontWeight:800,
                color:liq.col }}>{liq.badge}</div>
              <div style={{ fontSize:9, color:'var(--muted)' }}>
                {liq.detail}
              </div>
            </div>
            <div style={{ display:'grid',
              ...
              {[
                ...
                {lbl:'Vol%',   val:`${(liq.volR*100).toFixed(0)}%`,
                 col:liq.volR>1.8?A.red:A.blue},
                {lbl:'Status', val:liq.type==='normal'?'OK'
                 :liq.type.includes('sweep')?'SWEEP':'CLIMAX', col:liq.col},
              ].map((m,i)=>(
          ...
          <div>
            ...
            <div style={{ display:'grid',
              ...
              {plan.scenarios.map((sc2,i)=>(
                <div key={i} style={{
                  background:sc2.active&&sc2.type!=='avoid'
                    ?sc2.col+'10':'var(--card)',
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between',
                    ...
                    <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                      <div style={{ width:26, height:26, borderRadius:8,
                        ...
                        color:sc2.active?'#080c14':'var(--muted)',
                        flexShrink:0 }}>{sc2.id}</div>
                      <div>
                        <div style={{ fontSize:11, fontWeight:800,
                          color:sc2.active?sc2.col:'var(--muted)' }}>
                          {sc2.name}
                        </div>
                    ...
                    <div style={{ textAlign:'center',
                      background:sc2.col+'18', borderRadius:7,
                      padding:'4px 8px', minWidth:44 }}>
                      <div style={{ fontSize:16, fontWeight:800,
                        color:sc2.col }}>{sc2.prob}%</div>
                      <div style={{ fontSize:8,
                        color:'var(--muted)' }}>prob</div>
                    </div>
                  </div>

                  <div style={{ background:sc2.col+'08', borderRadius:7,
                    padding:'7px 9px', marginBottom:7,
                    border:`1px solid ${sc2.col}22` }}>
                    <div style={{ fontSize:9, fontWeight:700,
                      color:sc2.col, marginBottom:2 }}>📌 LUẬN ĐIỂM</div>
                    <div style={{ fontSize:10, color:'var(--text)',
                      lineHeight:1.6 }}>{sc2.thesis}</div>
                  </div>
                  ...
                  {sc2.type!=='avoid'&&(
                    <div style={{ display:'grid',
                      ...
                      {[
                        {lbl:'📍 Entry',
                         val:`${sc2.entry.low}–${sc2.entry.high}`,col:sc2.col},
                        {lbl:'🛑 SL',  val:`${sc2.sl}`,  col:A.red},
                        {lbl:'🎯 TP1', val:`${sc2.tp1}`, col:A.green},
                        {lbl:'🎯 TP2', val:`${sc2.tp2}`, col:A.teal},
                        {lbl:'📊 R:R',
                         val:`1:${sc2.entry.rr}`,
                         col:sc2.entry.rr>=2?A.green:A.amber},
                        {lbl:'📈 Vol', val:'>1.2×TB', col:A.blue},
                      ].map((p,j)=>(
                        <div key={j} style={{ background:'var(--card2)',
                          borderRadius:5, padding:'4px 7px',
                          border:'1px solid var(--border)' }}>
                          <div style={{ fontSize:8,
                            color:'var(--muted)' }}>{p.lbl}</div>
                          <div style={{ fontSize:10, fontWeight:700,
                            color:p.col,
                  ...
                  {sc2.confirmations.map((c,j)=>(
                    <div key={j} style={{ display:'flex', gap:6,
                      alignItems:'center', padding:'3px 7px',
                      background:c.ok?A.green+'08':A.red+'08',
                      borderRadius:4, marginBottom:3 }}>
                      <span style={{ fontSize:10 }}>{c.ok?'✅':'❌'}</span>
                      <div style={{ flex:1 }}>
                        <span style={{ fontSize:9, fontWeight:700,
                          color:c.ok?'var(--text)':'var(--muted)' }}>
                          {c.lbl}
                        </span>
                        <span style={{ fontSize:9, color:'var(--muted)',
                          marginLeft:5 }}>{c.note}</span>
                      </div>
                    </div>
                  ))}

                  <div style={{ background:A.red+'08', borderRadius:6,
                    padding:'5px 8px', border:`1px solid ${A.red}22`,
                    marginTop:4 }}>
                    <div style={{ fontSize:9, fontWeight:700,
                      color:A.red }}>⛔ Vô hiệu hóa:</div>
                    <div style={{ fontSize:9,
                      color:'var(--muted)' }}>{sc2.invalidation}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <Card glow={A.cyan}>
            ...
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'separate',
                borderSpacing:'0 3px', fontSize:9 }}>
                <thead>
                  <tr>
                ...
                <tbody>
                  {plan.keyLevels.map((lv,i)=>{
                    ...
                    return (
                      <tr key={i} style={{
                        background:near?`${lv.col}12`
                          :i%2===0?'var(--card2)':'transparent' }}>
                        <td style={{ padding:'5px 7px',
                          borderRadius:'5px 0 0 5px' }}>
                          <span style={{ fontWeight:700,
                            color:lv.col }}>{lv.icon} {lv.lbl}</span>
                        </td>
                        <td style={{ padding:'5px 7px' }}>
                          ...
                          {near&&(
                            <span style={{ marginLeft:4,
                              background:A.amber+'22', color:A.amber,
                              borderRadius:3, padding:'1px 4px',
                              fontSize:7 }}>NOW</span>
                          )}
                        </td>
                        <td style={{ padding:'5px 7px', textAlign:'center' }}>
                          <span style={{ background:lv.col+'18',
                            color:lv.col, borderRadius:4,
                            padding:'1px 6px', fontSize:7,
                            fontWeight:700 }}>{lv.type}</span>
                        </td>
                        <td style={{ padding:'5px 7px', textAlign:'center',
          ...
          <Card glow={A.purple}>
            ...
            <div style={{ display:'grid',
              ...
              {[
                {ss:sc.conv,
                 entry:`$${ez.z1.low}–$${ez.z1.high}`,
                 mgmt:'50%→thêm 50% Vol xác nhận',
                 tp1:`$${s.tp1||6.32}`,
                 tp2:`$${sc.conv.tp}`},
                {ss:sc.div,
                 entry:'KHÔNG vào lệnh',
                 mgmt:'Theo dõi Vol',
                 tp1:'–', tp2:'–'},
                {ss:sc.brk,
                 entry:'THOÁT toàn bộ',
                ...
                <div key={i} style={{ background:ss.col+'12',
                  border:`1.5px solid ${ss.col}55`,
                  borderRadius:10, padding:'11px' }}>
                  <div style={{ display:'flex', justifyContent:'space-between',
                    marginBottom:5 }}>
                    <div style={{ fontSize:11, fontWeight:800,
                      color:ss.col }}>{ss.label}</div>
                    <Chip label={`${ss.prob}%`} color={ss.col} size={10} />
                  </div>
                  <div style={{ fontSize:10, color:'var(--text)',
                    marginBottom:6 }}>{ss.detail}</div>
                  <div style={{ background:ss.col+'08', borderRadius:7,
                    padding:'7px 9px' }}>
                    {[{l:'Entry',v:entry},{l:'Quản lý',v:mgmt},
                      {l:'TP1',v:tp1},{l:'TP2',v:tp2}].map((row,j)=>(
                      <div key={j} style={{ fontSize:9, marginBottom:3 }}>
                        <span style={{ color:'var(--muted)' }}>{row.l}: </span>
                        <span style={{ color:'var(--text)' }}>{row.v}</span>
                      </div>
                    ))}
                  </div>
