...
export function calcTradeReadiness({ verdict, ew, vsa, wyckoff, lme, calendar, cot }) {
  ...
  const nextHighImpactEvent = events
    .filter(e => e.impact === 'high' && Number.isFinite(Number(e.minutesUntil)) && Number(e.minutesUntil) >= 0)
    .sort((a, b) => (a.minutesUntil ?? 9999) - (b.minutesUntil ?? 9999))[0];

  const hoursUntilEvent = nextHighImpactEvent?.minutesUntil != null
    ? nextHighImpactEvent.minutesUntil / 60
    : 999;
  const calendarLive = calendar?.source && calendar.source !== 'fallback';
  const lmeLive = lme?.source && lme.source !== 'fallback';
  const cotLive = cot?.source && cot.source !== 'fallback';
  ...
  const checks = [
    ...
    { id: 'event', label: 'Không event cao trong 4h', ok: calendarLive && hoursUntilEvent > 4,
      value: !calendarLive ? 'Chưa có calendar live' : hoursUntilEvent < 999 ? `${nextHighImpactEvent.name} ${Math.round(hoursUntilEvent*60)}'` : 'Rảnh' },
    { id: 'lme', label: 'Tồn kho LME giảm', ok: lmeLive && Number(lme.change) < 0,
      value: !lmeLive ? 'Chưa có LME live' : Number.isFinite(Number(lme?.change)) ? `${Number(lme.change) > 0 ? '+' : ''}${(Number(lme.change)/1000).toFixed(1)}k MT` : 'N/A' },
    { id: 'cot', label: 'COT smart money net long', ok: cotLive && Number(cot.net_mm) > 0,
      value: !cotLive ? 'Chưa xác thực COT' : Number.isFinite(Number(cot?.net_mm)) ? `${Number(cot.net_mm) > 0 ? '+' : ''}${(Number(cot.net_mm)/1000).toFixed(1)}k` : 'N/A' },
  ];
...
export function simulateEventImpact(eventName) {
  ...
  const key = name.includes('cpi') || name.includes('consumer price') ? 'CPI'
    : name.includes('fed') || name.includes('fomc') ? 'Fed Rate'
    : name.includes('china') && (name.includes('pmi') || name.includes('industrial') || name.includes('manufacturing')) ? 'China PMI'
    : null;
  if (!key) return null;
...
export function calcSmartMoneyDivergence(cot) {
  ...
  const commLong  = numberOrFallback(cot?.comm_long, 45000);
  const commShort = numberOrFallback(cot?.comm_short, 118000);
  const mmLong    = numberOrFallback(cot?.mm_long, 62400);
  const mmShort   = numberOrFallback(cot?.mm_short, 18200);
  const nrLong    = numberOrFallback(cot?.nr_long, 20000);
  const nrShort   = numberOrFallback(cot?.nr_short, 18000);
  const oiChange  = numberOrFallback(cot?.oi_change_pct, 8);
  ...
  return {
    ...
    nonReportable: { long: nrLong, short: nrShort, net: nrLong - nrShort },
    commZScore,
    divergence,
    signal: divergence
      ? 'Absorption pattern — Commercials short nặng nhưng giá không giảm, Managed Money tăng long'
      : 'Không có divergence rõ rệt',
    openInterestTrend: oiChange,
    source: cot?.source || 'fallback',
    isFallback: cot?.source === 'fallback' || !cot,
  };
}
