...
export function buildDriverBreakdown({ pk1Score, pk2Score, dxyChg, fearGreed, blackSwanRisk, weights }) {
  ...
  const drivers = [
    ...
    // SỬA BUG: score đã là (100 - risk) — tức risk cao thì score thấp, ĐÃ
    // đúng hướng "bearish". Nếu weight còn là số âm nữa sẽ LẬT DẤU LẦN 2
    // (điểm thấp × trọng số âm = kết quả dương) — khiến risk càng cao lại
    // càng kéo verdict LÊN, ngược với ý nghĩa. weight ở đây PHẢI dương.
    mkDriver('blackswan', 'Black swan', 100 - safeNum(blackSwanRisk, 0), Math.abs(weights?.blackswan ?? 0.15)),
  ];
