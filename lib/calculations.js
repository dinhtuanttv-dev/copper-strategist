...
export const getTxt = data => {
  ...
  return t;
};// ─── Resample OHLCV daily → weekly/monthly ────────────────────────────────────
export const resampleOHLCV = (dailyBars, targetTF) => {
  if (!dailyBars?.length) return [];
  if (targetTF === 'D' || targetTF === 'H4' || targetTF === 'H1' || targetTF === 'M15')
    return dailyBars;
  ...
  return Object.entries(groups)
    ...
    .map(([key, bars]) => ({
      ...
      comex: bars[bars.length-1].comex,
      open:  bars[0].comex,
      high:  Math.max(...bars.map(b=>b.high||b.comex)),
      low:   Math.min(...bars.map(b=>b.low||b.comex)),
      vol:   bars.reduce((s,b)=>s+(b.vol||0),0),
    }));
};

// ─── Short Setup Calculator ────────────────────────────────────────────────────
export const calcShortSetup = (s, ew, vsa, verdict, imSignals, atr) => {
  const dxy      = imSignals?.DXY;
  const dxyBear  = dxy && parseFloat(dxy.chg||0) > 0.3;   // DXY tăng → Cu giảm
  const shfe     = imSignals?.SHFE;
  const shfeBear = shfe && parseFloat(shfe.chg||0) < -1.0;

  // ─── Điều kiện bắt buộc ───────────────────────────────────────────────────
  const cond1 = verdict.final < 40;
  const cond2 = ew.failure || ew.wave==='5' || ew.wave==='F';
  const cond3 = vsa.bearish || ['UPTHRUST','NO_DEMAND'].includes(vsa.latestBar?.vsa);
  const cond4 = dxyBear;
  const mandatoryOK = cond1 && cond2 && cond3 && cond4;
  ...
  const active    = mandatoryOK && bonus >= 1;
  const strength  = active ? Math.min(100, Math.round((4-verdict.final/10)*8+bonus*8)) : 0;
  const atv       = atr || s.atr || 0.12;
  const cu        = s.comex || 6.07;

  // ─── Price levels ─────────────────────────────────────────────────────────
  const entryHigh  = +(cu + atv*0.3).toFixed(3);
  const entryLow   = +(cu - atv*0.2).toFixed(3);
  const sl         = +(cu + atv*1.5).toFixed(3);
  const tp1        = +(cu - (sl-cu)*1.0).toFixed(3);
  const tp2        = +(cu - (sl-cu)*2.0).toFixed(3);
  const tp3        = +(cu - (sl-cu)*3.0).toFixed(3);
  const rr         = sl > cu ? +((cu-tp1)/(sl-cu)).toFixed(1) : 0;

  return {
    active, strength, mandatoryOK, bonus,
    conditions: { cond1, cond2, cond3, cond4 },
    entry: { high:entryHigh, low:entryLow },
    sl, tp1, tp2, tp3, rr,
    trigger: `H4 đóng dưới $${entryLow} + Vol > 1.2×TB`,
    invalidation: `H4 đóng trên $${sl}`,
    thesis: `W${ew.wave} failure + VSA ${vsa.meta?.short||'Bear'} + DXY tăng → short zone $${entryLow}–$${entryHigh}`,
  };
};