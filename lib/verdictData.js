...
export async function fetchCOTData() {
  ...
  try {
    const res = await fetch('/api/cot?force=1', { signal: AbortSignal.timeout(10000) });
...
export async function fetchEconomicCalendar() {
  ...
  try {
    const res = await fetch('/api/calendar?force=1', { signal: AbortSignal.timeout(8000) });
    ...
    return {
      events: [
        { name:'CPI (CPI Mỹ YoY)', impact:'high', forecast:'3.1%', prev:'3.3%', affectsCu:'bearish_if_high', isFallback:true },
        { name:'Fed Rate Decision (Fed Minutes)', impact:'high', affectsCu:'bullish_if_dovish', isFallback:true },
        { name:'ISM Manufacturing PMI', impact:'medium', forecast:'48.5', prev:'48.7', affectsCu:'bullish_if_beat', isFallback:true },
        { name:'China PMI (Caixin China PMI)', impact:'high', forecast:'51.2', prev:'50.8', affectsCu:'bullish_if_beat', isFallback:true },
      ],
